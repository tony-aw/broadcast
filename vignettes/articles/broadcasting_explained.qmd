...
In 'R' this can be done manually, or using `outer() |> drop()`:
...
When `x` and `y` are small arrays, like in the above example, there is hardly a problem; but when `x` and `y` become larger, the 3 matrices (`x` is recycled to a matrix, `y` is recycled to a matrix, and the result is also matrix) also become larger.
...
In an operation like `outer(x, y, "+")`, `x` and `y` are recycled to become the same size. The thing is, recycling of `x` and `y` does not need to happen physically; they only need to be recycled virtually.  

Virtual recycling does not actually recycle `x` and `y`. Instead, nested loops in 'C' and 'C++' are used to simulate a recycled array.  
...
And **that** is what broadcasting does: broadcasting provides **fast** virtual recycling of array dimenisons in the context of operations involving 2 (or more) arrays **without** allocating additional memory.
