...
library(fontawesome) |> suppressMessages()
```
...
Another example is the 'xtensor' package (and extensions to that package, like 'rray'), that allows `r fa("r-project")` - users to access the 'C++' module of the same name.
...
These alternatives all need access to a different programming language, which brings with it a host of problems:
 ...
 - **No access to CRAN's quality control**:  `r fa("r-project")` is unique in that it has access to CRAN's quality control; most other languages (including Python) don't have a real comparable organization like CRAN.
...
The 'broadcast' package adheres to the 'tinyverse' philosophy (not to be confused with the 'tidyverse'), which is the programming philosophy to primarily minimize dependencies. 'broadcast' also belongs to the [fastverse](https://github.com/fastverse/fastverse), which is a collection of packages aimed at keeping code fast and efficient.

Packages that belong to the 'fastverse' can be found listed on the dedicated GitHub page:  
[https://github.com/fastverse/fastverse](https://github.com/fastverse/fastverse).

Other `r fa("r-project")` packages that adhere to the philosophy of the 'tinyverse' include, but are not limited to, the following:
 ...
 - [rrapply](https://github.com/JorisChau/rrapply): for recursive manipulation of lists.
 ...
 - [qs](https://github.com/qsbase/qs): provides fast saving, loading, and serialization of `r fa("r-project")` objects.
 - [this.path](https://github.com/ArcadeAntics/this.path): provides a consistent mechanism for an R script to retrieve its own path.
 - and **many** others.
