...
<pre><code class='language-R'>acast(x, ...)
...
and providing an unbalanced factor for <code>grp</code> produces an error.
</td></tr>
...
The <code>fill_val</code> argument is ignored when <code>fill = FALSE</code>. <br>
If <code>fill_val</code> is missing, it is specified as follows: <br>


<ul>
<li> If <code>x</code> is of type <code>raw</code> and <code>fill = TRUE</code>, <code>fill_val</code> is not allowed to be missing, and an error is returned;

</li>
<li> If <code>x</code> is atomic but not <code>raw</code>, <code>fill_val</code> is set to <code>NA</code>;

</li>
<li> If <code>x</code> is of type <code>list</code>, <code>fill_val</code> is set to <code>list(NULL)</code>. <br> <br>
...
to get a good idea on how this function casts an array. <br> <br>



### Value

An array with dimensions <code style="white-space: pre;">c(dim(x), max(tabulate(grp))</code>. <br> <br>
