...
However, if, for example, `x` is a long vector, and `y` is a directional vector with direction `2`, an error is returned: this is because arrays in `r fa("r-project")` cannot have dimensions larger than `2^31 - 1`.
