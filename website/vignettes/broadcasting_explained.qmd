...
To compute the element-wise addition using broadcasting through the 'broadcast' package, one can set both `x` and `y` as "broadcaster", and then use the `+` operator, like so:
...
We now take again 2 matrices `x` and `y`, but this time `x` is a column vector (i.e. a matrix with 1 column and multiple rows), and `y` is a row vector (i.e. a matrix with 1 row and multiple columns):
...
For example, if `dim(x) = c(3, 4, 3)` and `dim(y) = c(3, 4)`, then internally `dim(y)` is interpreted to be `c(3, 4, 1)`. This neither copies nor modifies array `y`.

Following the standard rules of Linear Algebra, a vector `z` without dimensions is taken to be a column vector (i.e. a matrix with 1 column and `length(z)` rows).
...
As you can see, performing a broadcasted operation between a column- and row- vector (in that order) is equivalent to using `outer()` on 2 dimensionless vectors.
...
Broadcasting can thus be used as a more efficient alternative to `outer()` in case of vector inputs, simply by making the inputs orthogonal vectors.
