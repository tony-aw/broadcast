...
In the documentation of 'broadcast', the reader may occasionally find references to the "direction" of a vector. The direction of a vector can mean several different things, depending on the context. Before going further, we need to address what the direction of a vector means in the context of broadcasting.
...
A directional vector is an array where only one dimension is larger than 1.
The direction of a vector refers to **which** dimension is larger than 1.

`z` is a directional vector, and its direction is 2.

Now the most important part:  
Following the standard rules of Linear Algebra, a vector `z` without dimensions is interpreted by the 'broadcast' package as a vector with direction `1`.
...
Since dimensionless vectors are interpreted as directional vectors (which is necessary to meaningfully perform broadcasting), when may wonder: how does the 'broadcast' package deal with  `long` vectors?
...
However, if, for example, `x` is a long vector, and `y` is an array (regardless if it is a directional vector or not), an error is returned: this is because arrays in `r fa("r-project")` cannot have dimensions larger than `2^31 - 1`.

::: {.callout-tip}

One can easily create a directional vector using the [vector2array()](/man/vector2array.qmd) function provided by 'broadcast'.
...
Following the standard rules of Linear Algebra, a vector `z` without dimensions is taken to be a directional vector with direction 1.
...
Consider the directional vectors `x` and `y`, where their directions are `1` and `2`, respectively:
...
As you can see, performing a broadcasted operation between vectors with directions 1 and 2 (in that order) is equivalent to using `outer()` on 2 dimensionless vectors.
...
Broadcasting can thus be used as a more efficient alternative to `outer()` in case of vector inputs, simply by making the inputs orthogonal directional vectors.
