...
Packages like 'mgcv' provide ways to plot and analyse the smoothers from a fitted GAM with confidence intervals.  
But sometimes one may want to compute spline intervals manually, like in the following cases:
 ...
 - One may want to use a type of spline **not** covered by the package you're using (this is especially true when using a novel spline). In such cases, you'll have to compute the spline means and confidence/credible intervals on your own.
...
$$
where $Q_\text{Norm}$ is the quantile function of the standard Normal distribution, CDF is the cumulative distribution function of the model, with model parameters $\mu_i$ and $\theta$, and $y_i$ is the response value at observation $i$.
...
Some important problems on computing Dunn-Smyth residuals via simulation-based empirically approximated CDFs:
 ...
 - Cumulative probabilities based on simulations are **approximations**. The more simulated values, the better the approximations. But exact Dunn-Smyth residuals are virtually always more accurate than those based on simulated values.
 ...
 - Just like the parametrization of a model can be poorly documented, how a model generates simulated values can also be poorly documented.
