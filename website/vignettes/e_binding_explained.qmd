...
[bind_array()](/man/bind.qmd) fully supports broadcasting. And the `ndim2bc` argument (an abbreviation of " maximum number of dimensions to broadcast") allows the users to specify how many dimensions are allowed to be broadcasting while binding arrays. This way, users won't get unpleasant surprises.
...
By default, `ndim2bc` is set to `1`. This means that, by default, [bind_array()](/man/bind.qmd) will broadcast no more than 1 dimension per array in the input list, when necessary.
