...
Outer computations is a common set of operations used in Linear Algebra, and base `r fa("r-project")` provides the `outer()` function to facilitate such computations. But `outer()` is not at all an efficient function.
