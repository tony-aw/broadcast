...
Well, as great as `abind::abind()` is, there are some issues with it, and the [bind_array()](/man/bind.qmd) function from 'broadcast` attempts (and in my humble opinion succeeds) to provide a better alternative:

 - `abind()` is a bit slow. [bind_array()](/man/bind.qmd) is significantly faster.
 - `abind()` does not support broadcasting. [bind_array()](/man/bind.qmd) does support broadcasting.
 - `abind()` does not support recursive arrays (i.e. arrays of type `list`). When binding recursive arrays, `abind()` will attempt to unlist it, even when it does not make sense, ruining the structure of the array. [bind_array()](/man/bind.qmd) fully supports recursive arrays, as well as arrays of all atomic types.
 - `abind()` has somewhat confusing mechanisms for giving dimension names to the output. [bind_array()](/man/bind.qmd) has more streamlined naming arguments.


[bind_array()](/man/bind.qmd) uses a similar API to that of `abind::abind()`, and provides similar features like binding arrays before the first dimension (using `along = 0`) or after the last dimension.
