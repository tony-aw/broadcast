...
First, create a modification broadcaster vector, whose dimensional orientation is aligned with - in this case - the columns; this can be done easily using, for example, `%orientbc<-%` (see the help page of [vector2array()](/man/vector2array.qmd) for details).  
Second, make `x` a broadcaster, and perform the broadcasted addition between the array and oriented vector.
...
```{r}
v %orientbc<-% 3L # change orientation of oriented vector to 3L
print(v)
bcr(x) <- TRUE
