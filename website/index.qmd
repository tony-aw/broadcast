...
Second, 'broadcast' provides the [bind_array()](/man/bind_array.qmd) function, which is a broadcasted and enhanced form of the fantastic `abind::abind()` function:

-   [bind_array()](/man/bind_array.qmd) allows for broadcasting (obviously), whereas `abind::abind()` does not.
-   [bind_array()](/man/bind_array.qmd) is significantly **faster** and uses **less memory** than `abind::abind()`.
-   [bind_array()](/man/bind_array.qmd) also differs from `abind::abind()` in that it can handle recursive arrays properly; `abind::abind()` unlists everything to atomic arrays, ruining the structure.
