...
As show below, this cannot be done efficiently in base 'R'; it **can** be done fast and memory-efficiently with the 'broadcast' package:

::: {.grid}

::: {.g-col-6}

In base `r fa("r-project")`:
...
Using broadcasting, `bind_array()` from the 'broadcast' package can bind arrays together in ways that cannot efficiently be done with `rbind()`, `cbind()`, or `abind::abind()`. Let's consider these arrays:
...
As show in the table below, this cannot be done efficiently in base 'R'; it **can** be done fast and memory-efficiently with the 'broadcast' package:
...
`bind_array()` is also considerably faster and more memory efficient than `abind()`. See the [benchmarks](https://tony-aw.github.io/broadcast/about/f_benchmarks_other.html).
...
Second, turn the shallow (i.e. non-nested), dimensional list into an atomic array using `cast_shallow2atomic()`:
