...
all_byx_decl <- sprintf("const int by_x%d = INTEGER_RO(by_x)[%d];\t\\", 1:16, 0:15)
all_byy_decl <- sprintf("const int by_y%d = INTEGER_RO(by_y)[%d];\t\\", 1:16, 0:15)
all_dcpx_decl <- sprintf("const double dcp_x%d = REAL_RO(dcp_x)[%d];\t\\", 1:16, 0:15)
all_dcpy_decl <- sprintf("const double dcp_y%d = REAL_RO(dcp_y)[%d];\t\\", 1:16, 0:15)
...
forx <- c(
  "\t\\",
  sprintf("i_x%d = by_x%d * iter%d * dcp_x%d + i_x%d;\t\\",
          2:16, 2:16, 2:16, 2:16, 3:17)
)
fory <- c(
  "\t\\",
  sprintf("i_y%d = by_y%d * iter%d * dcp_y%d + i_y%d;\t\\",
          2:16, 2:16, 2:16, 2:16, 3:17)
)
...
#define MACRO_DIM_<dtype>(DOCODE) do {      \\
  ...
  R_xlen_t <all_y_decl>; \\
  <startfor>
        flatind_x = by_x1 * iter1 + i_x2;       \\
        flatind_y = by_y1 * iter1 + i_y2;     \\
                                                                    \\
        DOCODE;                                                          \\
  	                                                                \\
        flatind_out++;                      \\
...
for(i in DTYPES) {
  ...
  current_byx_decl <- stri_c(all_byx_decl[1:i], collapse = "\n")
  current_byy_decl <- stri_c(all_byy_decl[1:i], collapse = "\n")
  # note on dcp: dcp_x1 and dcp_y1 are not used, hence I start at 2:
  current_dcpx_decl <- stri_c(all_dcpx_decl[2:i], collapse = "\n")
  current_dcpy_decl <- stri_c(all_dcpy_decl[2:i], collapse = "\n")
...
#define MACRO_DIM_DEC_<dtype>(DOCODE) do {      \\
  R_xlen_t flatind_out = 0;         \\
...
macro_dim_dec_d <- stri_c(dMacro_skeletons, collapse = "\n")
...
all_byy_decl <- sprintf("const int by_y%d = INTEGER_RO(by_y)[%d];\t\\", 1:16, 0:15)
all_dcpx_decl <- sprintf("const double dcp_x%d = REAL_RO(dcp_x)[%d];\t\\", 1:16, 0:15)
all_dcpout_decl <- sprintf("const double dcp_out%d = REAL_RO(dcp_out)[%d];\t\\", 1:16, 0:15)
...
forout <- c(
  "\t\\",
  sprintf("i_out%d = iter%d * dcp_out%d + i_out%d;\t\\", 2:16, 2:16, 2:16, 3:17)
)
forx <- c(
  "\t\\",
  sprintf("i_x%d = by_x%d * (iter%d - start%d) * dcp_x%d + i_x%d;\t\\",
          2:16, 2:16, 2:16, 2:16, 2:16, 3:17)
)
...
#define MACRO_DIM_BIND_<dtype>(DOCODE) do {  \\
  ...
  <startfor>
        ...
        flatind_x = by_x1 * (iter1 - start1) + i_x2;           \\
        DOCODE;                         \\
  <endfor>
} while(0)
...
for(i in BINDTYPES) {
  ...
  current_byx_decl <- stri_c(all_byx_decl[1:i], collapse = "\n")
  # note on dcp: dcp_x1 and dcp_out1 are not used, hence I start at 2:
  current_dcpx_decl <- stri_c(all_dcpx_decl[2:i], collapse = "\n")
  current_dcpout_decl <- stri_c(all_dcpout_decl[2:i], collapse = "\n")
