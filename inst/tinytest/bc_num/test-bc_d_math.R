      ...
      for(iDataX in 1:length(x.data)) { # different data types for x
        ...
        for(iDataY in 1:length(y.data)) { # different data types for y
          ...
          if(is.null(tdim)) {
            ...
            expected[[i]] <- as.double(x) + array_replicate(as_dbl(y), tdim)
            out[[i]] <- bc.d(x, y, op)
          }
          else if(length(y) == 1L && length(x) > 1L) {
            # CASE 4: y is scalar, x is not
            expected[[i]] <- array_replicate(as_dbl(x), tdim) + as.double(y)
            out[[i]] <- bc.d(x, y, op)
          }
          else {
            # CASE 5: x and y are both non-reducible arrays
            expected[[i]] <- array_replicate(as_dbl(x), tdim) + array_replicate(as_dbl(y), tdim)
            out[[i]] <- bc.d(x, y, op)
          }
          # END CASES
      ...
      for(iDataX in 1:length(x.data)) { # different data types for x
        ...
        for(iDataY in 1:length(y.data)) { # different data types for y
          ...
          if(is.null(tdim)) {
            ...
            # CASE 3: x is scalar, y is not
            expected[[i]] <- pmin(as.double(x), array_replicate(as_dbl(y), tdim))
            out[[i]] <- bc.d(x, y, op)
          }
          else if(length(y) == 1L && length(x) > 1L) {
            # CASE 4: y is scalar, x is not
            expected[[i]] <- pmin(array_replicate(as_dbl(x), tdim), as.double(y))
            out[[i]] <- bc.d(x, y, op)
          }
          else {
            # CASE 5: x and y are both non-reducible arrays
            expected[[i]] <- pmin(array_replicate(as_dbl(x), tdim), array_replicate(as_dbl(y), tdim))
            out[[i]] <- bc.d(x, y, op)
          }
          # END CASES
      ...
      for(iDataX in 1:length(x.data)) { # different data types for x
        ...
        for(iDataY in 1:length(y.data)) { # different data types for y
          ...
          if(is.null(tdim)) {
            ...
            # CASE 3: x is scalar, y is not
            expected[[i]] <- pmax(as.double(x), array_replicate(as_dbl(y), tdim))
            out[[i]] <- bc.d(x, y, op)
          }
          else if(length(y) == 1L && length(x) > 1L) {
            # CASE 4: y is scalar, x is not
            expected[[i]] <- pmax(array_replicate(as_dbl(x), tdim), as.double(y))
            out[[i]] <- bc.d(x, y, op)
          }
          else {
            # CASE 5: x and y are both non-reducible arrays
            expected[[i]] <- pmax(array_replicate(as_dbl(x), tdim), array_replicate(as_dbl(y), tdim))
            out[[i]] <- bc.d(x, y, op)
          }
          # END CASES
