...
i <- 1L
x.data <- sample(c(TRUE, FALSE, NA), 100, TRUE)
    ...
    for(iDimY in c(1, 2, 5, 8, 9)) { # different dimensions for y
      ...
      for(iDataY in 1:length(y.data)) { # different data types for y
        ...
        if(is.null(tdim)) {
          ...
          expected[[i]] <- basefun(as.integer(x), array_recycle(as_int(y), tdim))
          out[[i]] <- bc.b(x, y, op)
        }
        else if(length(y) == 1L && length(x) > 1L) {
          # CASE 4: y is scalar, x is not
          expected[[i]] <- basefun(array_recycle(as_int(x), tdim), as.integer(y))
          out[[i]] <- bc.b(x, y, op)
        }
        else {
          # CASE 5: x and y are both non-reducible arrays
          expected[[i]] <- basefun(array_recycle(as_int(x), tdim), array_recycle(as_int(y), tdim))
          out[[i]] <- bc.b(x, y, op)
        }
        # END CASES
...
i <- 1L
y.data <- sample(c(TRUE, FALSE, NA), 100, TRUE)
basefun <- function(x, y) {
  out <- x == y
  return(out)
}
